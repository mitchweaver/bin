#!/usr/bin/env bash
#
# A shell script to preview markdown files in html with your browser.
#
...
while [ $# -gt 0 ] ; do
    ...
    case "$1" in
        --help|-h)
            printf "%b%b\n%b%b\n%b%b%b%b%b\n\n" \
                "View a markdown file in html wth your browser.\n" \
                "-------------------------------------------------------\n" \
                "Usage: ./previewmarkdown.sh /path/to/file.md\n" \
                "-------------------------------------------------------\n" \
                "Options:\n" \
                "-------------------------------------------------------\n" \
                "--browser, -b    -   Which browser to use (default: xdg-open)\n" \
                "--input,   -i    -   The input file to be converted and opened.\n" \
        ...
        --input|-i)
            if [ -n "$2" ] ; then
                ...
                # This is causing issues in some cases,
                # look into it later.
                #
                # elif [ ! -r "$1" ] ; then 
                #     printf "%s\n%s\n\n" \
                #         "The \"$2\" file cannot be read!" \
...
##### TO DO: make this posix / eliminate bash dependency
convert() {
...
result=$(curl -s https://api.github.com/markdown -d "{\"text\": \"$text\", \"mode\": \"gfm\", \"context\": \"\"}")
